...
def train(model, data, configs):
	# scaffold init (singleton class)
	fl_scaffold = FLScaffold(model)
	fl_scaffold.set_global(model)
	fl_scaffold.get_controls()
	...
	for epoch in range(EPOCHS):
		...
		for batch_idx, (image, label) in enumerate(train_data):
			...
			# scaffold apply
			fl_scaffold.scaffold_apply(model, optimizer)
	...
	# scaffold update
	fl_scaffold.scaffold_update(model, local_steps)
